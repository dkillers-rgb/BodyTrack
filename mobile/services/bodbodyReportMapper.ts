...
import { calcSegmentMusclePercent } from '../utils/segmentBodyArt';
...
function segPart(
  ...
  return {
    muscle,
    fat,
    musclePercent: calcSegmentMusclePercent(muscle, skeletalMuscle, segment),
  };
}
...
export function normalizeBodbodyReport(raw: unknown): BodbodyReportSnapshot | null {
  ...
  if (r.section5 && typeof r.section5 === 'object' && 'activeTypeIndex' in (r.section5 as object)) {
    ...
    const pickStatus = (patterns: string[]): EvalStatus => {
      const row = old6?.find((item) => patterns.some((p) => item.label.toLowerCase().includes(p)));
      return row ? mapOldStatus(row.status) : 'normal';
    };

    const section5: BodbodyReportSnapshot['section5'] = [
      { label: REPORT_LABELS.evalWeight, status: pickStatus(['peso', 'weight']) },
      { label: REPORT_LABELS.evalMuscle, status: pickStatus(['músculo', 'muscle', 'skeletal']) },
      { label: REPORT_LABELS.evalBodyFat, status: pickStatus(['gordura corporal', 'body fat', 'gordura']) },
      { label: REPORT_LABELS.evalObesity, status: pickStatus(['obesidade', 'obesity', 'imc']) },
    ];
...
function defaultRanges(weight: number, heightCm: number, gender: Client['gender']) {
...
export function mapCodeValueToBodbodyReport(codeValue: unknown): BodbodyReportSnapshot {
  ...
  const laM = toNumber(v[I.SEG_LA_M]) ?? skeletalMuscle * 0.1;
  const raM = toNumber(v[I.SEG_RA_M]) ?? skeletalMuscle * 0.1;
  const trM = toNumber(v[I.SEG_TRUNK_M]) ?? skeletalMuscle * 0.45;
  const llM = toNumber(v[I.SEG_LL_M]) ?? skeletalMuscle * 0.22;
  const rlM = toNumber(v[I.SEG_RL_M]) ?? skeletalMuscle * 0.22;
  ...
  return {
    ...
    section4: {
      leftArm: segPart(laM, toNumber(v[I.SEG_LA_F]) ?? bodyFatKg * 0.08, skeletalMuscle, 'leftArm'),
      rightArm: segPart(raM, toNumber(v[I.SEG_RA_F]) ?? bodyFatKg * 0.08, skeletalMuscle, 'rightArm'),
      trunk: segPart(trM, toNumber(v[I.SEG_TRUNK_F]) ?? bodyFatKg * 0.5, skeletalMuscle, 'trunk'),
      leftLeg: segPart(llM, toNumber(v[I.SEG_LL_F]) ?? bodyFatKg * 0.18, skeletalMuscle, 'leftLeg'),
      rightLeg: segPart(rlM, toNumber(v[I.SEG_RL_F]) ?? bodyFatKg * 0.18, skeletalMuscle, 'rightLeg'),
    },
    section5: buildComprehensive(
      weight,
      skeletalMuscle,
      bodyFatPct,
      bmi,
