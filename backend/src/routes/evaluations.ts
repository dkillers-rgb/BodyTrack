...
router.post('/scan-qr', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erro ao processar QR Code';
...
router.post('/auto-save', upload.single('image'), async (req: Request, res: Response) => {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erro no processamento automático';
    res.status(500).json({ error: message });
  }
});
